...
(function($, doc){
	...
	*/
	...
	$.photobox = function(_images, startImage, _options){
	...
	$.fn.photobox = function(_options, linkMapper, linksFilter){
		linkMapper = linkMapper || function(el){
			return [el.href, el.firstChild.getAttribute('alt') || el.firstChild.getAttribute('title')];
		};

		linksFilter = linksFilter || function(){
			return true;
		};
		// all the links that this script should be applied to
		imageLinks = this;
		...
		// save reference on the delegated element about all the thumbnails.
		$(doc).on('click', this.selector, onClick);//.data(this.selector, imageLinks);
		
		function onClick(e){
			e.preventDefault();

			//imageLinks = $(e.delegateTarget).data(e.handleObj.selector);
			
			var link = this, linksMapped = [];
			// Build the list of images that will be displayed
			filteredLinks = $.grep(imageLinks, function(el){
			...
			if( linksMapped.length < 2 ){
				$([prevBtn, nextBtn]).remove();
			}
			
			// need this for later:
			docElm = doc.documentElement;
			return $.photobox(linksMapped, $.inArray(this, imageLinks), _options);
		};
	...
	function changeImage(imageIndex, firstTime){
		if( imageIndex >= 0 ){
			...
			if( options.thumbs ){
				activeThumb.removeClass('active');
				activeThumb = thumbs.find('li').eq(imageIndex).addClass('active');
			}
